...
import { Footprints, Timer, Ruler, MapPin } from 'lucide-react'
...
export default function TeroWalksSection() {
  const { lang } = useLang()
  const tx = t[lang].hiking
  const descs = tx.descs as Record<string, string>
  const ref = useRef(null)
  const isInView = useInView(ref, { once: true, margin: '-60px 0px' })

  return (
    <section id="balades" ref={ref} className="py-12 px-4 bg-white">
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex-1 rounded-xl bg-white/70 p-2.5 text-center">
                    ...
                    <p className="font-bold text-stone-700 text-sm">{route.walkTime}</p>
                    <p className="text-stone-400 text-[10px]">{tx.walk}</p>
                  </div>
                  <div className="flex-1 rounded-xl bg-white/70 p-2.5 text-center">
                    <Timer size={13} className="text-stone-400 mx-auto mb-1" />
                    <p className="font-bold text-stone-700 text-sm">{route.runTime}</p>
                    <p className="text-stone-400 text-[10px]">{tx.run}</p>
                  </div>
                </div>
              </motion.div>
            )
